...
bool DAQUtilities::AddPort(std::string ServiceName, unsigned int port){
  ...
  std::stringstream test;
  test<<"PortAdd "<< ServiceName <<" "<<m_UUID<<" "<<port<<" "<<((int)false) ;
...
bool DAQUtilities::RemovePort(std::string ServiceName){
  
  zmq::socket_t Ireceive (*context, ZMQ_PUSH);
  Ireceive.connect("inproc://ServicePublish");
  
  std::stringstream test;
  test<<"PortDelete "<< ServiceName << " ";
  zmq::message_t send(test.str().length()+1);
  snprintf ((char *) send.data(), test.str().length()+1 , "%s" ,test.str().c_str()) ;
...
int DAQUtilities::UpdateConnections(std::string ServiceName, zmq::socket_t* sock, std::map<std::string,Store*> &connections, std::string port, std::string port_name){
